...
| **ENERGY**                                            | Every power plant of 1 MW and more (782, SFOE register, hydro named from WASTA) by type and capacity, brightened by the type's current output; the 220/380 kV grid (ESTI); the storage lakes' filling per region (SFOE, weekly); Swissgrid live import and export as animated arrows and grid frequency; Energy-Charts mix and day-ahead price; CHARTS shows the day's mix and two years of reservoir filling. | 60 s / 15 min / weekly |
...
pnpm --filter @swiss-now/core build-data energy                   # plants ≥ 1 MW (SFOE), WASTA names, 220/380 kV grid → apps/web/public/data/energy
