...
| ---------------------------- | ---------------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------------------- |
| `gtfs.yml`                   | Mon, Thu 16:00 CET (after the 15:00 RT switch) | 240 MB zip, minutes of CPU                 | `rail/patterns.json`, `rail/paths/*.json`, `rail/stops.json` → Blob                 |
| `snapshot.yml`               | every 10 min                                   | history must exist when nobody is watching | `snapshots/{ts}.json`, `state/summary.json` → Blob; aggregates → Supabase (Phase 3) |
