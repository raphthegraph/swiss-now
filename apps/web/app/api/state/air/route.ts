...
import { AIR_TTL_SECONDS, getAirState } from "@/lib/state/air";
import { cacheControl } from "@/lib/state/weather";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** AirState: Zürich reference stations, citizen particulate sensors, pollen. */
export async function GET(req: Request) {
  try {
    const state = await getAirState();
    const { updatedAt: _u, ...stable } = state;
    const etag = `"${createHash("sha1").update(JSON.stringify(stable)).digest("hex").slice(0, 16)}"`;
    const headers = {
      "Cache-Control": cacheControl(AIR_TTL_SECONDS),
      ETag: etag,
