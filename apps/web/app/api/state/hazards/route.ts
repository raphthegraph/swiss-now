...
import { HAZARDS_TTL_SECONDS, getHazardsState } from "@/lib/state/hazards";
import { cacheControl } from "@/lib/state/weather";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** HazardsState: forest-fire and avalanche regions, snow stations, hail. */
export async function GET(req: Request) {
  try {
    const state = await getHazardsState();
    const { updatedAt: _u, ...stable } = state;
    const etag = `"${createHash("sha1").update(JSON.stringify(stable)).digest("hex").slice(0, 16)}"`;
    const headers = {
      "Cache-Control": cacheControl(HAZARDS_TTL_SECONDS),
      ETag: etag,
